...
class CCPRestSDK {
    ...
    {
        $this->Filename = ROOT_DIR ."/log/ccp_rest.log";
     ...
     * 设置主帐号
     *
     * @param $AccountSid string 主帐号
     * @param $AccountToken string 主帐号Token
     */
     ...
     * 设置子帐号
     *
     * @param $SubAccountSid string 子帐号
     * @param $SubAccountToken string 子帐号Token
     * @param $VoIPAccount string VoIP帐号
     * @param $VoIPPassword string VoIP密码
     */
    ...
    /**
     * 创建子帐号
     * @param $friendlyName string 子帐号名称
    ...
    /**
     * 获取子帐号
     * @param $startNo integer 开始的序号，默认从0开始
     * @param $offset integer 一次查询的最大条数，最小是1条，最大是100条
     * @return mixed|SimpleXMLElement|stdClass
     */
...
//        }
     ...
     * @param $msgDomain string
     * @param $msgFileName string
     * @param $msgFileUrl string
     * @return bool|mixed|stdClass|\SimpleXMLElement
     */
    function PushMsg($pushType, $msgType, $sender, $receivers, $msgContent, $msgDomain = null, $msgFileName = null, $msgFileUrl = null)
    {
        //主帐号鉴权信息验证，对必选参数进行判空。
        $auth=$this->accAuth();
        if($auth!=""){
        ...
        // 拼接请求包体
        if($this->BodyType=="json"){
            $bodyArr = array('appId' => $this->AppId, 'pushType'=>$pushType, 'sender' => $sender,
                'msgType' => $msgType, 'msgContent' => $msgContent, 'receiver' => $receivers);
            if(isset($msgDomain)){ $bodyArr['msgDomain'] = $msgDomain; }
            if(isset($msgFileName)){ $bodyArr['msgFileName'] = $msgFileName; }
            if(isset($msgFileUrl)){ $bodyArr['msgFileUrl'] = $msgFileUrl; }
            $body = json_encode($bodyArr);
        }else{
            $this->showlog("Only json supported");
            return false;
        }
        // 大写的sig参数
        $sig =  strtoupper(md5($this->AccountSid . $this->AccountToken . $this->Batch));
        ...
        //$url = "http://develop.qst.org/test.php";
        $this->showlog("request url = ".$url);
        // 生成授权：主帐户Id + 英文冒号 + 时间戳。
        $authen = base64_encode($this->AccountSid . ":" . $this->Batch);
        $this->showlog("authen = ".$authen);
        // 生成包头
        $header = array("Accept:application/$this->BodyType","Content-Type:application/$this->BodyType;charset=utf-8","Authorization:$authen");
        // 发送请求
        $result = $this->curl_post($url,$body,$header);
        $this->showlog("request body = ".$body);
        $this->showlog("request header = ".json_encode($header));
        $this->showlog("request url = $url");
        $this->showlog("response body = ".$result);
    ...
    /**
     * @param $groupid string 容联群组id
     * @return stdClass
     */
    function QueryMember($groupid)
    {
        //主帐号鉴权信息验证，对必选参数进行判空。
        $auth=$this->subAuth();
        if($auth!=""){
        ...
        //拼接请求包体
        if('json' == $this->BodyType){
            $body = json_encode(array('groupId'=>$groupid));
        }else{
            $body="<Request>
