...
/*     *
     ...
     */
function bank_render($body){
    $html = '<!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd">';
    $html .= "<html>";
    $html .= "<head>";
    $html .= '<meta http-equiv="Content-Type" content="text/html; charset=utf-8">';
    $html .= "<title>支付宝纯网关接口接口</title>";
    $html .= "</head>";

    require_once(dirname(dirname(__FILE__)) . "/lib/alipay.config.php");
    require_once(dirname(dirname(__FILE__)) . "/lib/alipay_submit_bank.class.php");
    /* ************************请求参数************************* */
    ...
    //页面跳转同步通知页面路径
    $return_url = $body["alipay_return_url"];//"http://商户网关地址/create_direct_pay_by_user-PHP-UTF-8/return_url.php";
    ...
    //必填，银行简码请参考接口技术文档
    //默认网银
    $defaultbank = $alipay_config['WIDdefaultbank'];
    $subject = _PAYSUBJECT_;
    // 日志
    $info = 'alipaybank.php,operator success,order_no:' . $out_trade_no
        . ',totalfee:' . $total_fee . ',banktype:' . $defaultbank;
    tracelog($info);
